...
export default function handler(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield runMiddleware(req, res, upload.single("certificate"));
            if (!req.file) {
            ...
            try {
                ...
                try {
                    ...
                    return res.status(400).json({
                        ...
                        details: {
                            error: "Could not parse the certificate. Please ensure it is in PEM or DER format.",
                        },
                    });
                }
            }
            ...
            if (cert.extensions) {
                cert.extensions.forEach((ext) => {
                    if (ext.name === "keyUsage" && ext.digitalSignature) {
                        keyUsage = Object.keys(ext).filter((key) => typeof ext[key] === "boolean" &&
                            ext[key] === true &&
                            key !== "critical");
                    }
                        ...
                        subjectAltName = ext.altNames
                            .map((an) => `${an.type === 2 ? "DNS" : an.type === 7 ? "IP" : "Outro"}:${an.value}`)
                            .join(", ");
                    }
                });
            }
            ...
            const fingerprint = ((_a = md.digest().toHex().match(/.{2}/g)) === null || _a === void 0 ? void 0 : _a.join(":")) || "";
            // Translate key usage terms to English
            const translateKeyUsage = (usage) => {
                const translations = {
